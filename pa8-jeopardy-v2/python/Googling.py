...
class Googling:
    ...
    def guessLocation(self, data):
        ...
        other = "<LOCATION>(\w+)</LOCATION>"
        us = "<LOCATION>([\w\s]+)</LOCATION>[\s,]*<LOCATION>([\w\s]+)</LOCATION>"
        
        #print "snip", data[0].snip
        
        for d in data:
            
            snip = re.sub('<em>', '', d.snip)
            snip = re.sub('</em>', '', snip)
            #d = re.sub('[Ss]erver', '', d)
            #print "snip", snip
            
            matches = re.findall(us, snip)
            if len(matches) > 0:
                #print snip, matches[0][0], ",", matches[0][1]
                return Location(matches[0][0],matches[0][1])
        
        #matches = re.findall(other, data[0].snip)
        #if len(matches) > 0:
        #    print matches[0][0], ",", matches[0][1]
        #    return Location(matches[0][0],matches[0][1])
        
        
        #print data[0].title, data[0].snip
